...
- [x] 02-03-PLAN.md — Gap closure: fix silent exit on fresh system (ensure_dirs + OSError handler)
...
- [x] 03-03-PLAN.md — meet summarize CLI command with session resolution, map-reduce chunking, metadata extension, tests
...
Plans:
- [x] 04-01-PLAN.md — Notion service module (create_page, extract_title, markdown_to_blocks, text splitting, retry logic) + NotionConfig + tests
- [x] 04-02-PLAN.md — Integrate Notion push into summarize command + NotionTokenCheck/NotionDatabaseCheck health checks + doctor registration + tests
...
- [x] 05-02-PLAN.md — meet list command with Rich table, --status filter, --json output, duration/title derivation
